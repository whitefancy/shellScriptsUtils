...
tree -dL 1 /etc/ #<==-d参数只显示目录,-L参数显示层数,这里是1层。

#例子五:用tree命令来区分目录和文件的方法
tree -L 1 -F /boot/ #<==使用-F参数会在目录后面添加“/”,方便区分目录

tree -L 1 -F /boot/|grep /$ #<==过滤以斜线结尾的所有内容
tree -L 1 -d /boot/ #<==使用-d参数只显示目录树,这样可轻松过滤内容中的目录<br>/boot/<br>|-- efi<br>|-- grub<br>'-- lost+found<br>3 directories
